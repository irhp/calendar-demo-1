...
var DragListener = fc.DragListener = Class.extend({
		...
		if (!this.isListening) {
			...
			$(document)
				...
				.on('selectstart', this.preventDefault) // prevents native selection in IE<=8
 				.on('touchmove', this.touchmoveProxy = proxy(this, 'touchmove'))
 				.on('touchend', this.touchendProxy = proxy(this, 'touchend'))
				...
				//this.originY = ev.pageY;
 				if(ev.type === "touchstart") {
 					if(ev.originalEvent.touches.length === 1) {
 						ev.originX = ev.originalEvent.touches[0].pageX;
 						ev.originY = ev.originalEvent.touches[0].pageY;
 					}
 				}
 				else
 				{
	...
	//CHANGE:
 	...
 	touchmove: function(ev) {
 		var dx = ev.originalEvent.touches[0].pageX - this.originX;
 		var dy = ev.originalEvent.touches[0].pageY - this.originY;
	...
	stopListening: function(ev) {
		...
		if (this.isListening) {
			...
			$(document)
				//CHANGE:
 				.off('touchmove', this.touchmoveProxy)
				.off('mousemove', this.mousemoveProxy)
				.off('mouseup', this.mouseupProxy)
				.off('selectstart', this.preventDefault);

			//CHANGE:
 			this.touchmoveProxy = null;
...
var CellDragListener = DragListener.extend({
	...
	listenStart: function(ev) {
		...
		if (ev) {
			...
			//origPoint = { left: ev.pageX, top: ev.pageY };
 			var coordX, coordY;
 			if(ev.type === 'touchstart' && ev.originalEvent.touches.length === 1) {
 				coordX = ev.originalEvent.touches[0].pageX;
 				coordY = ev.originalEvent.touches[0].pageY;
 			}
 			else {
 				coordX = ev.pageX;
 				coordY = ev.pageY;
 			...
 			origPoint = { left: coordX, top: coordY}
			point = origPoint;
	...
	dragStart: function(ev) {
		...
		//cell = this.getCell(ev.pageX, ev.pageY); // might be different from this.origCell if the min-distance is large
 		var coordX, coordY;
 		if(ev.type === 'touchmove' && ev.originalEvent.touches.length === 1) {
 			coordX = ev.originalEvent.touches[0].pageX;
 			coordY = ev.originalEvent.touches[0].pageY;
 		}
 		else {
	...
	drag: function(dx, dy, ev) {
		...
		/***********************************/
		//This is the original code that we removed
...
var MouseFollower = Class.extend({
	...
	start: function(ev) {
		if (!this.isFollowing) {
			...
			$(document).on('mousemove', this.mousemoveProxy = proxy(this, 'mousemove'));
 			$(document).on('touchmove', this.touchmoveProxy= proxy(this, 'touchmove'));
		}
	},
	...
	//CHANGE:
 	touchmove: function(ev) {
 		this.topDelta = ev.originalEvent.touches[0].pageY - this.mouseY0;
 		this.leftDelta = ev.originalEvent.touches[0].pageX - this.mouseX0;
...
Grid.mixin({
	...
	bindSegHandlers: function() {
		...
		$.each(
			{
				//CHANGE:
 				touchstart: function(seg, ev) {
 					if ($(ev.target).is('.fc-resizer') && view.isEventResizable(seg.event)) {
 						_this.segResizeMousedown(seg, ev, $(ev.target).is('.fc-start-resizer'));
 					}
 					else if(view.isEventDraggable(seg.event)) {
 						_this.segDragTouchstart(seg,ev);
 					}
 				},
 
 				touchend: function(seg, ev) {
	...
	//CHANGE:
 	triggerSegTouchstart: function(seg, ev) {
	...
	//Change:
 	segDragTouchstart: function(seg, ev) {
 		var _this = this;
 		var view = this.view;
 		...
 		dragListener.touchstart(ev); // start listening, which will eventually lead to a dragStart
 	},
	...
	renderSegPopoverContent: function(cell, segs) {
		...
		var content = $(
			...
			'<div class="fc-body ' + view.widgetContentClass + '">' +
				'<div class="fc-event-container">1</div>' +
			'</div>'
		);
